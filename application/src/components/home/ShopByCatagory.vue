<template>
  <div>
    <v-container class="my-6">
      <h2 class="heading-primary my-2" align="center">
        shop by
        <span class="heading-primary-span">catagories</span>
      </h2>
      <p class="catagory-text" align="center">
        Lorem ipsum, dolor sit amet consectetur adipisicing elit. Aliquam velit
        pariatur magnam fuga maiores
      </p>
      <v-row class="mt-5">
        <v-col cols="12" md="3" sm="12" xs="12" align="center">
          <div class="cat-vegetables">
            <img src="@/assets/img/cat-veg.png" alt="Vegetables" class="cat-vegetables__photo" />
            <h3 class="heading-tertiary heading-tertiary__mod mb-2">vegetables</h3>
            <p class="cat-vegetables__text">
              When transitioning to a healthier lifestyle, it’s helpful to begin
              the journey.
            </p>
          </div>
        </v-col>
        <v-col cols="12" md="3" sm="12" xs="12" align="center">
          <div class="cat-vegetables">
            <img src="@/assets/img/cat-veg.png" alt="Vegetables" class="cat-vegetables__photo" />
            <h3 class="heading-tertiary heading-tertiary__mod mb-2">vegetables</h3>
            <p class="cat-vegetables__text">
              When transitioning to a healthier lifestyle, it’s helpful to begin
              the journey.
            </p>
          </div>
        </v-col>
        <v-col cols="12" md="3" sm="12" xs="12" align="center">
          <div class="cat-vegetables">
            <img src="@/assets/img/cat-veg.png" alt="Vegetables" class="cat-vegetables__photo" />
            <h3 class="heading-tertiary heading-tertiary__mod mb-2">vegetables</h3>
            <p class="cat-vegetables__text">
              When transitioning to a healthier lifestyle, it’s helpful to begin
              the journey.
            </p>
          </div>
        </v-col>
        <v-col cols="12" md="3" sm="12" xs="12" align="center">
          <div class="cat-vegetables">
            <img src="@/assets/img/cat-veg.png" alt="Vegetables" class="cat-vegetables__photo" />
            <h3 class="heading-tertiary heading-tertiary__mod mb-2">vegetables</h3>
            <p class="cat-vegetables__text">
              When transitioning to a healthier lifestyle, it’s helpful to begin
              the journey.
            </p>
          </div>
        </v-col>
      </v-row>
    </v-container>
  </div>
</template>

<script>
export default {};
</script>

<style></style>
